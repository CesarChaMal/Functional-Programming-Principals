...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filterNot(n => explored.exists(b => b == n._1))
  }
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    if (initial.isEmpty) Stream.Empty
      ...
      val next = for {
        s <- initial
        n <- newNeighborsOnly(neighborsWithHistory(s._1, s._2), explored)
  ...
  lazy val solution: List[Move] = {
    pathsToGoal.foldLeft(List[Move]())((best, c) => if (best.length == 0 || c._2.length < best.length) c._2 else best).reverse
  }
}
